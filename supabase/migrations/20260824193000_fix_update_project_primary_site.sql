-- Qualify project_sites.project_id in the update RPC.
-- RETURNS TABLE (project_id uuid, slug text) made the unaliased column
-- reference in UPDATE ... WHERE project_id = p_project_id ambiguous.
-- Also expose primary-site coordinates without sending raw geometry through PostgREST.
...
create or replace function public.primary_site_coordinates(p_project_id uuid)
returns table (
...
as $$
  select
    extensions.st_y(s.geom) as latitude,
    extensions.st_x(s.geom) as longitude,
    s.location
  from public.project_sites as s
  where s.project_id = p_project_id
    and s.is_primary
...
revoke all on function public.primary_site_coordinates(uuid) from public, anon;
grant execute on function public.primary_site_coordinates(uuid) to authenticated;
