-- Contamination / environmental-history intelligence:
-- Länsstyrelserna EBH Potentiellt förorenade områden (external point layer).
-- Screening-level official records only — not contamination confirmation,
-- remediation design, cost, or liability.

-- ---------------------------------------------------------------------------
-- Allow Point/MultiPoint features alongside MultiPolygon catalog rows
-- ---------------------------------------------------------------------------
...
comment on table public.official_geographic_features is
  'Official Swedish geographic reference features for screening (protected, Natura 2000, mapped flood, mapped ground, mapped contamination). Not cadastral parcels. Mapped contamination is EBH potentially contaminated-site points — screening-level environmental-history evidence, not a contamination confirmation.';
...
insert into public.grid_sources (
  ...
  'lst-ebh-potentiellt-fororenade',
  'gis',
  'Länsstyrelserna / EBH-stödet',
...
-- ---------------------------------------------------------------------------
-- Candidate contamination metrics
-- ---------------------------------------------------------------------------
alter table public.opportunity_run_candidates
  add column if not exists contamination_queried boolean not null default false,
  add column if not exists contamination_intersecting_count integer,
  add column if not exists contamination_nearby_count integer,
  add column if not exists contamination_nearest_m numeric,
  add column if not exists contamination_risk_classes text[] not null default '{}'::text[],
  add column if not exists contamination_statuses text[] not null default '{}'::text[],
  add column if not exists contamination_record_ids text[] not null default '{}'::text[],
  add column if not exists contamination_provider_key text;

comment on column public.opportunity_run_candidates.contamination_queried is
  'True when Search Area EBH windows were evaluated. False = UNKNOWN. Zero records with queried=true is evaluated absence in dataset, not missing evidence.';
comment on column public.opportunity_run_candidates.contamination_intersecting_count is
  'Count of official EBH points intersecting the Candidate footprint. Screening only — not confirmed contamination.';
comment on column public.opportunity_run_candidates.contamination_nearby_count is
  'Count of official EBH points within 180 m of the Candidate that do not intersect the footprint. NOXHEIM screening assumption.';
comment on column public.opportunity_run_candidates.contamination_nearest_m is
  'Nearest official EBH point distance in metres (0 when intersecting).';

alter table public.opportunity_searches
  add column if not exists contamination_mode text not null default 'preference';

alter table public.opportunity_searches
  drop constraint if exists opportunity_searches_contamination_mode_check;
alter table public.opportunity_searches
  add constraint opportunity_searches_contamination_mode_check
  check (contamination_mode in ('hard', 'preference'));

comment on column public.opportunity_searches.contamination_mode is
  'NOXHEIM screening assumption: preference = risk/major_risk on intersecting or nearby EBH records; hard = intersecting becomes BLOCKER. Not a remediation or legal standard.';

-- ---------------------------------------------------------------------------
-- Upsert: allow mapped_contamination (points skip polygon clip extract)
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Apply contamination metrics after site segmentation
-- Nearby distance: 180 m (NOXHEIM screening assumption)
-- ---------------------------------------------------------------------------
create or replace function public.apply_contamination_to_run(p_run_id uuid)
returns void
language plpgsql
security definer
set search_path = ''
...
declare
  ...
  v_contamination_evaluated boolean;
  v_nearby_m double precision := 180.0;
begin
  ...
  into v_contamination_evaluated;

  update public.opportunity_run_candidates as c
  set
    contamination_queried = coalesce(v_contamination_evaluated, false),
    contamination_provider_key = case
      when coalesce(v_contamination_evaluated, false) then 'lst-ebh-potentiellt-fororenade'
      else null
    end,
    contamination_intersecting_count = case
      when not coalesce(v_contamination_evaluated, false) then null
      else coalesce((
    ...
    contamination_nearby_count = case
      when not coalesce(v_contamination_evaluated, false) then null
      else coalesce((
        select count(*)::integer
        from public.official_geographic_features as f
        where f.feature_class = 'mapped_contamination'
          and f.geom operator(extensions.&&) extensions.st_expand(c.geom, 0.01)
          and extensions.st_dwithin(f.geom::extensions.geography, c.geom::extensions.geography, v_nearby_m)
          and not extensions.st_intersects(f.geom, c.geom)
      ), 0)
    end,
    contamination_nearest_m = case
      when not coalesce(v_contamination_evaluated, false) then null
      else (
    ...
    contamination_risk_classes = case
      when not coalesce(v_contamination_evaluated, false) then '{}'::text[]
      else coalesce((
        select array_agg(distinct cls order by cls)
        from (
          select coalesce(
            nullif(trim(f.properties->>'riskClass'), ''),
          ...
          where f.feature_class = 'mapped_contamination'
            and f.geom operator(extensions.&&) extensions.st_expand(c.geom, 0.01)
            and (
              extensions.st_intersects(f.geom, c.geom)
              or extensions.st_dwithin(f.geom::extensions.geography, c.geom::extensions.geography, v_nearby_m)
            )
        ) as classes
      ), '{}'::text[])
    end,
    contamination_statuses = case
      when not coalesce(v_contamination_evaluated, false) then '{}'::text[]
      else coalesce((
        select array_agg(distinct st order by st)
        from (
          select nullif(trim(f.properties->>'status'), '') as st
          from public.official_geographic_features as f
          where f.feature_class = 'mapped_contamination'
            and f.geom operator(extensions.&&) extensions.st_expand(c.geom, 0.01)
            and (
            ...
            and nullif(trim(f.properties->>'status'), '') is not null
        ) as statuses
      ), '{}'::text[])
    end,
    contamination_record_ids = case
      when not coalesce(v_contamination_evaluated, false) then '{}'::text[]
      else coalesce((
        select array_agg(distinct rid order by rid)
        from (
          select coalesce(nullif(trim(f.properties->>'ebhId'), ''), f.external_id) as rid
          from public.official_geographic_features as f
          where f.feature_class = 'mapped_contamination'
            and f.geom operator(extensions.&&) extensions.st_expand(c.geom, 0.01)
            and (
...
revoke all on function public.apply_contamination_to_run(uuid) from public, anon;
grant execute on function public.apply_contamination_to_run(uuid) to authenticated, service_role;

-- ---------------------------------------------------------------------------
-- Coverage RPC: add contamination window status (preserve flood/ground)
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- Covered windows with zero EBH points remain covered (evaluated empty).
  if exists (
    select 1 from public.official_ingest_windows as w
    where w.source_slug = 'lst-ebh-potentiellt-fororenade'
      and w.status in ('covered', 'partial')
...
comment on function public.get_search_area_coverage(double precision, double precision, double precision, double precision) is
  'Coverage of official discovery layers for a Search Area bbox. Flood, ground and contamination use ingest windows. Protected/Natura/Ei/NUP are national catalog presence.';

comment on function public.apply_contamination_to_run(uuid) is
  'Sets Candidate EBH environmental-history metrics from cached mapped_contamination points. Zero records with queried=true is evaluated, not missing.';
