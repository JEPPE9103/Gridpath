...
**Stage A2 — Site generation v2** (`site-generation-v2`) places deterministic SWEREF 99 TM grid seeds inside each Opportunity Zone, scores them from supported land-cover / terrain / protected-boundary evidence, grows a site by buffering the seed to the customer **target site area** (clipped to remaining usable geography), stops at **maximum candidate area**, drops sites below **minimum contiguous usable area**, then deduplicates pairs with IoU ≥ 0.5. Extra hectares above the configured target do not add ranking benefit (`suitability-v4` target-fit). The user-facing object is a **Candidate Site**. A surviving county-scale remainder is never returned as a site.
...
Candidate-site methodology (`site-generation-v2`): analysis cells → ST_Difference of configured exclusions → ST_UnaryUnion / ST_Dump **Opportunity Zones** → deterministic grid seeds scored from land-cover, terrain, and protected-boundary distance → buffer-grow clipped to usable geography to the configured target (cap at max) → IoU ≥ 0.5 dedupe → top-N. Decision metric inside a site remains largest contiguous usable area vs configured minimum. Extra hectares above target do not dominate `suitability-v4` ranking. Detailed refinement may further subtract ingested-tile excluded land-cover / steep terrain and persist pre-refinement geometry.
