...
**Stage A — Discovery screening** divides the box into square **analysis cells** in SWEREF 99 TM (EPSG:3006). Cell size is `clamp(2000, 10000, sqrt(area_m² / 200))` metres, targeting about 200 cells. Coarse (~1 km) physical summaries may be used. Official exclusion polygons are subtracted with `ST_Difference`. Remaining usable polygons that share a boundary are dissolved (`ST_UnaryUnion` + `ST_Dump`) into **Opportunity Zones** (regional continuity — not investigation targets).

**Stage A2 — Site generation v2.1** (`site-generation-v2.1`) keeps Opportunity Zones from dissolve, then covers each zone with a 150 m SWEREF 99 TM grid of units clipped to remaining usable land. Units inherit land-cover and terrain evidence; excluded classes are ineligible. A seed is the highest-scoring unused eligible unit. The site is region-grown 4-connected through eligible neighbours until the configured **target** is reached, neighbours run out, or **maximum candidate area** is hit. The selected units are dissolved. Target area is a preference, not a cookie-cutter radius. Geometry quality reviews corridors, necks, fragmentation and envelope fill; it does **not** prefer circles. Extra hectares above target do not add ranking benefit (`suitability-v4` target-fit). The user-facing object is a **Candidate Site**. A surviving county-scale remainder is never returned as a site.

Customer site-area fields (or labelled NOXHEIM defaults): minimum contiguous usable **8 ha**, target **15 ha**, maximum candidate **30 ha**, maximum returned **25** (hard cap 100).

**Stage B — Detailed site screening** runs only on selected Candidate Sites (max 5). It uses ingested land-cover composition tiles (majority-class from native 10 m, targeting 50 m and capped at 100 m) and, when Geotorget is configured, on-demand Lantmäteriet 1 m DTM tiles. Pre-refinement geometry is retained. 1 km majority class is never presented as final site evidence. Processing resolution is labelled separately from the 10 m source.
...
| Natura 2000 | Naturvårdsverket | National shapefile download `https://geodata.naturvardsverket.se/nedladdning/naturvardsregistret/` (`SPA_Rikstackande.zip`, `SCI_Rikstackande.zip`); WFS is fallback only | CC0 | Yes | Preferred: “Källa: Naturvårdsverket” | Bulk download ingest. Feature count must be > 0 before the layer is treated as evaluated. |
| NMD 2023 basskikt v0.3 | Naturvårdsverket | Nationwide GeoTIFF `https://geodata.naturvardsverket.se/nedladdning/marktacke/NMD2023/Basskikt_v0_x/` | CC0 | Yes | Preferred: “NMD2023 v0.3, Naturvårdsverket” | Production: host a **Cloud Optimized GeoTIFF** and set server-only `NOXHEIM_NMD2023_URL` (HTTP range/window reads via `geotiff` `fromUrl`). One-time convert: `node scripts/convert-nmd-to-cog.mjs`. Never download the full ~1.3 GB striped TIF per search. Operator/dev may use `NOXHEIM_NMD2023_TIF` / `NOXHEIM_GEODATA_CACHE`. Discovery: 1 km majority class. Precision: majority-class tiles targeting 50 m (cap 100 m). Mapping `nmd-group-v2`. |
...
| Lantmäteriet Markhöjdmodell 1 m | Lantmäteriet | STAC `https://api.lantmateriet.se/stac-hojd/v1` | Product terms via Geotorget | Requires authorised Geotorget access | Lantmäteriet | Preferred detailed terrain. Candidate-scoped tiles only. `AUTH_REQUIRED` / `FALLBACK_ACTIVE` until credentials exist. Copernicus DSM is not Swedish 1 m DTM. |
| Lantmäteriet Marktäcke Nedladdning, vektor | Lantmäteriet | STAC `https://api.lantmateriet.se/stac-vektor/v1/collections/marktacke` (GeoPackage) | Avgiftsfri / valuable-dataset terms via Geotorget | Requires authorised Geotorget access | Lantmäteriet | Complementary topographic land types. **Does not replace NMD.** `AUTH_REQUIRED` until Geotorget credentials exist. |
...
| INSPIRE RoadLink (NVDB) | Trafikverket | Lastkajen GeoPackage (`TRAFIKVERKET_ROADLINK_GPKG`); WFS is not retried per search after HTTP 400 | CC0 | Yes | Trafikverket NVDB | Until Lastkajen is configured, road context stays insufficient evidence and does not block site generation. OSM is not substituted. |
| MSB/MCF översvämningskartering (BHF) | Myndigheten för civilt försvar / MSB | INSPIRE WFS/WMS `https://inspire.mcf.se/geoserver/oversvamning/` (`NZ_Oversvamning_BHF`) | Open official / INSPIRE | Yes | “MSB/MCF översvämningskartering (BHF)” | On-demand Search Area windows. Prefers WFS vectors; falls back to official WMS window masks when vector payloads are too large. Empty window = evaluated with no mapped overlap in this dataset — **not** absent flood exposure. Not hydrological modelling. |
| SGU Jordarter 1:25 000–1:100 000 (grundlager) | Sveriges geologiska undersökning (SGU) | OGC API Features `https://api.sgu.se/oppnadata/jordarter25k-100k/ogc/features/v1` collection `grundlager` | CC0 1.0 | Yes | “SGU Jordarter 1:25 000–1:100 000” | On-demand Search Area windows. Surficial geology near mapping depth (~0.5 m). Map scale varies 1:25k–1:100k. Screening-level mapped composition — **not** a geotechnical site investigation. |
| Länsstyrelserna Potentiellt förorenade områden (EBH) | Länsstyrelserna / EBH-stödet | Official ZIP shapefile (SWEREF99 TM points) via `ext-dokument.lansstyrelsen.se` | Official länsstyrelserna geodata distribution | Yes | “Länsstyrelserna — Potentiellt förorenade områden (EBH)” | On-demand Search Area windows with disk ZIP cache. Point records; intersecting / 180 m nearby counts. Screening-level environmental-history evidence — **not** contamination confirmation, remediation, cost, or liability. Zero records with coverage = evaluated empty, not missing. |
| Malmö stad Gällande detaljplaner | Malmö stad · SBK | ArcGIS REST MapServer GeoJSON `https://gis.malmo.se/arcgis/rest/services/SEPlan/Gallande_planer/MapServer/1` | Official municipal GIS (SBK attribution) | Malmö only (coverage-aware) | “Malmö stad · Official Source” | First municipal planning layer. On-demand Search Area windows. Mapped plan boundaries + status/ID provenance. **Not** Sweden-wide. Göteborg/Örebro registered unavailable (no public machine-readable API). NGP Detaljplan is future AUTH_REQUIRED. Intersection is context, not zoning approval. Empty evaluated ≠ missing. |
...
Candidate-site methodology (`site-generation-v2.1`): analysis cells → ST_Difference of configured exclusions → ST_UnaryUnion / ST_Dump **Opportunity Zones** → 150 m eligible land units scored from land-cover and terrain → seed at the strongest unused unit → 4-connected region grow until target preference / no neighbour / max → dissolve selected units → IoU ≥ 0.5 safety-net dedupe → top-N. Decision metric inside a site remains largest contiguous usable area vs configured minimum. Extra hectares above target do not dominate `suitability-v4` ranking. Detailed refinement may further subtract ingested-tile excluded land-cover / steep terrain and persist pre-refinement geometry.

Ingest: `npm run dev:ingest-nv-protected`, `dev:ingest-nv-natura`, `dev:ingest-copernicus-slope -- --bbox=14.9,59.1,15.4,59.4`, `dev:ingest-nmd-2023 -- --bbox=...` (cached national TIF), `dev:ingest-scb-admin`, `dev:ingest-trafikverket-roads -- --bbox=...`, `dev:ingest-lantmateriet-dtm`, `dev:ingest-lantmateriet-marktacke`. Rasters are never stored in git. Lantmäteriet tiles and Marktäcke remain on-demand / AUTH_REQUIRED without Geotorget.
...
- Analysis cells are an internal discovery mechanism. Dissolved remaining geography is an **Opportunity Zone**. User-facing geometry is the **Candidate Site**; detailed screening may further change that geometry.
