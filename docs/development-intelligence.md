...
| Lantmäteriet Markhöjdmodell 1 m | Lantmäteriet | STAC `https://api.lantmateriet.se/stac-hojd/v1` | Product terms via Geotorget | Requires authorised Geotorget access | Lantmäteriet | Preferred detailed terrain via Sweden-wide on-demand Discovery. Server-only auth: OAuth2 `LANTMATERIET_CLIENT_ID`/`CLIENT_SECRET` (preferred), or `LANTMATERIET_STAC_TOKEN`, or Basic Geotorget username/password. Downloads only required STAC tiles for Search Area windows (`dtm1m`), derives ~100 m slope/elevation summaries into `official_precision_summaries`, then `apply_detailed_terrain_to_run`. Raw 1 m DTM is never stored in PostGIS or sent to the browser. `AUTH_REQUIRED` until credentials exist — Copernicus GLO-90 remains separately labelled coarse terrain. |
...
Terrain methodology: Discovery uses Horn slope on GLO-90 DSM samples (~1 km), labelled **coarse terrain**. Detailed Candidate Intelligence uses Lantmäteriet 1 m DTM when Geotorget is configured: on-demand STAC tile fetch → server-side Horn slope + elevation range summaries (~100 m cells) → Candidate metrics (mean/P90 slope, elev range, steep-area %). Coarse and detailed provenance stay separate. Slope/relief constraints use NOXHEIM screening assumptions (steep share / relief thresholds). Do not claim constructability, cut/fill volumes, grading cost, or foundation suitability.
