...
async function executeGeographicScreening(input: {
  ...
  const { error: dtmError } = await input.supabase.rpc("apply_detailed_terrain_to_run", {
    p_run_id: runRow.run_id,
  });
  if (dtmError) {
    console.error("executeGeographicScreening detailed terrain failed", dtmError.message);
    return {
      error: publicError(dtmError.message, "Screening ran but detailed terrain assessment failed."),
      values: input.values,
      searchId: input.searchId,
    };
  }
...
export async function rerunOpportunitySearchAction(formData: FormData): Promise<void> {
  ...
  const { error: dtmError } = await supabase.rpc("apply_detailed_terrain_to_run", {
    p_run_id: runRow.run_id,
  });
  if (dtmError) {
    console.error("rerunOpportunitySearchAction detailed terrain failed", dtmError.message);
    return;
  }
