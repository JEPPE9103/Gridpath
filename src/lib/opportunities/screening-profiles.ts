...
export type ScreeningProfileCriteria = {
  ...
  /** preference = clay/peat risk/major_risk; hard = BLOCKER. NOXHEIM screening assumption. */
...
export function parseScreeningProfileCriteria(raw: unknown): ScreeningProfileCriteria {
  ...
  return {
    ...
    groundMode: value.groundMode === "hard" ? "hard" : "preference",
    groundHardExclusionPct: numberOrNull(value.groundHardExclusionPct) ?? fallback.groundHardExclusionPct,
    groundClayRiskPct: numberOrNull(value.groundClayRiskPct) ?? fallback.groundClayRiskPct,
