/**
 * Multi-geography SGU ground product-path smoke (Örebro / Malmö / Göteborg).
 * Uses Design Partner Cloud. Does not reset demo.
 */
import { createClient } from "@supabase/supabase-js";
import { GROUND_SOURCE_SLUG } from "../src/lib/ingest/coverage-keys";
...
async function smokeOne(
  ...
  const groundSources = first.sources.filter((s) => s.slug === GROUND_SOURCE_SLUG);
  return {
    geography: geo.name,
    bbox: geo,
    groundStatus: first.coverage?.ground?.status ?? null,
    groundFeatures: first.coverage?.ground?.intersectingFeatures ?? null,
    firstMs,
    secondMs,
    firstGroundActions: groundSources.map((s) => s.action),
    firstFetchedGround: first.fetched.includes(GROUND_SOURCE_SLUG),
    secondReused: second.reusedCache,
    messages: first.messages.filter((m) => /ground|soil|sgu/i.test(m)),
  };
}
...
async function main() {
  ...
  console.log(JSON.stringify({ event: "smoke.ground.multi_geography", results }, null, 2));
  for (const row of results) {
    if (row.groundStatus !== "covered" && row.groundStatus !== "partial") {
      throw new Error(`${row.geography}: expected ground covered/partial, got ${row.groundStatus}`);
    }
  }
}
